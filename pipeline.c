 ...
 */
static void _etape_signaler(__etape_pipeline__ *etape, __etape_statut__ statut,
...
#define _DET_MAX_ETATS_AFDC                                                    \
  256 // max états dans l'AFDC (2^N peut exploser, on borne)
 ...
 */
static uint64_t _det_delta(const __automate_state__ *af, uint64_t masque,
                           char symbole) {
  uint64_t res = 0;
  size_t nb_tr = _DA_LENGTH(af->_transitions_);
    ...
    for (int s = 0; s < nb_sym; s++) {
      ...
      if (found == -1) {
        ...
        if (nb_etats_afdc >= _DET_MAX_ETATS_AFDC) {
          fprintf(stderr, "determinisation: trop d'états (%d max)\n",
                  _DET_MAX_ETATS_AFDC);
          return NULL;
        }
        found = nb_etats_afdc;
 ...
 */
static char *_min_nom_classe(const __automate_state__ *afdc, int *membres,
                             int nb_membres) {
  char buf[1024] = "";
...
static __automate_state__ *_op_minimiser(const __automate_state__ *afdc) {
  ...
  // ── Journal des partitions successives (stocké dans _log_, affiché par le
  // main) ── On évite tout printf ici : ce thread tourne en parallèle du main
  // thread qui peut lui-même être en train d'afficher. Tout texte produit
  // pendant le calcul est accumulé dans ce buffer puis transféré dans
  // afdcm->_log_.
  size_t log_cap = 4096;
  size_t log_len = 0;
...
#define _LOG_APPEND(fmt, ...)                                                  \
  do {                                                                         \
    int _n =                                                                   \
        snprintf(log_buf + log_len, log_cap - log_len, fmt, ##__VA_ARGS__);    \
    if (_n > 0) {                                                              \
      log_len += (size_t)_n;                                                   \
...
#undef _LOG_APPEND
  ...
  // transitions : pour chaque classe c, prendre un représentant et ses
  // transitions
...
// standardisation — repart de l'automate original
typedef struct {
...
// déterminisation — repart de l'automate original
typedef struct {
  const __automate_state__ *_af_;
  const __automate_tests__ *_tests_;
  __etape_pipeline__ *_etape_;
...
static int _thread_complementaire(void *arg) {
  ...
  while (a->_etape_minimisation_->_statut_ == ETAPE_EN_ATTENTE ||
    ...
    cnd_wait(&a->_etape_minimisation_->_cnd_, &a->_etape_minimisation_->_mtx_);
  }
...
__pipeline__ *pipeline_creer(const __automate_state__ *af) {
  ...
  if (!args_std || !args_det || !args_min || !args_cmp) {
    ...
    free(args_cmp);
    fprintf(stderr,
            "pipeline: impossible d'allouer les arguments des threads\n");
    goto erreur;
  }
  ...
  // Lancer les threads
  if (thrd_create(&p->_etape_standardisation_._thread_, _thread_standardisation,
                  args_std) != thrd_success) {
  ...
  if (thrd_create(&p->_etape_determinisation_._thread_, _thread_determinisation,
                  args_det) != thrd_success) {
  ...
  if (thrd_create(&p->_etape_minimisation_._thread_, _thread_minimisation,
                  args_min) != thrd_success) {
  ...
  if (thrd_create(&p->_etape_complementaire_._thread_, _thread_complementaire,
                  args_cmp) != thrd_success) {
