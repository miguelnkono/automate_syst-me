...
static int _thread_standardisation(void *arg)
{
    ...
    _etape_signaler(a->_etape_,
                    res != NULL ? ETAPE_PRETE : ETAPE_ERREUR,
                    res);
...
static int _thread_minimisation(void *arg)
{
    ...
    if (statut == ETAPE_IGNOREE && a->_tests_->est_deterministe && a->_tests_->est_complet)
