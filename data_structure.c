...
static inline void *_da_init(const size_t item_size, const size_t capacity) {
  __da_header__ *h =
      (__da_header__ *)malloc(item_size * capacity + sizeof(__da_header__));
...
/**
 * S'assurer que la liste a assez d'espace pour stocker `capacity_increase`
 * éléments supplémentaires.
 * @param da est un double pointeur vers la liste dynamique (pour permettre la
 * mise à jour de l'adresse en cas de realloc).
 * @param capacity_increase est le nombre d'éléments supplémentaires à
 * accueillir.
 */
static inline void _da_ensure_capacity(void **da,
  ...
  if (h->length + capacity_increase > h->capacity) {
    ...
    h = (__da_header__ *)realloc(h, h->item_size * new_capacity +
                                        sizeof(__da_header__));
...
__automate_state__ *automate_state_create(void) {
  __automate_state__ *as =
      (__automate_state__ *)malloc(sizeof(__automate_state__));
  if (as == NULL) {
