 ...
 * @param _cnd_         condition signalée quand _statut_ == ETAPE_PRETE | ETAPE_IGNOREE | ETAPE_ERREUR
 * @param _statut_      état courant de l'étape
 * @param _resultat_    automate produit par l'étape (NULL si pas encore prêt ou ignoré)
 */
 ...
 *
 * @param _af_original_          automate lu depuis le fichier (lecture seule par les threads)
 * @param _etape_standardisation_    T1 : standardisation si nécessaire
 * @param _etape_determinisation_    T2 : déterminisation + complétion
 * @param _etape_minimisation_       T3 : minimisation (attend T2)
 * @param _etape_complementaire_     T4 : complémentaire (attend T3)
 */
...
/**
 * Créer et initialiser un pipeline à partir d'un automate déjà chargé en mémoire.
 * Lance immédiatement tous les threads en arrière-plan.
 *
 * @param af  automate source (doit rester valide pendant toute la durée du pipeline)
 * @return    pointeur vers le pipeline, ou NULL en cas d'erreur
 */
__pipeline__ *pipeline_creer(const __automate_state__ *af);

/**
 * Attendre la fin de toutes les étapes et libérer toutes les ressources du pipeline.
 * Les __automate_state__ produits par les étapes sont également détruits.
 *
