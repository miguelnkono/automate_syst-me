 ...
 */
static void _cellule_transitions(const __automate_state__ *as, uint8_t etat,
                                 char symbole, char *tampon, size_t taille) {
  tampon[0] = '\0';
  size_t nb_transitions = _DA_LENGTH(as->_transitions_);

  for (size_t i = 0; i < nb_transitions; i++) {
    __automate_transition__ t =
        _DA_GET(__automate_transition__, as->_transitions_, i);
...
void afficher_tests(const __automate_tests__ *t) {
  ...
  if (t->est_standard) {
    ...
    if (t->nb_initiaux_exces > 0)
      printf("    → %d état(s) initial/initiaux en trop (doit en avoir "
             "exactement 1)\n",
             t->nb_initiaux_exces);
 ...
 */
static const char *_nom_etat(const __automate_state__ *as, int e, char *tampon,
                             size_t taille) {
...
void afficher_automate(const __automate_state__ *as) {
      ...
      if (as->_noms_etats_ != NULL) {
        ...
        for (size_t k = 0; k < nb_tr; k++) {
          ...
          if (t.etat_depart == (uint8_t)e && (char)t.symbole == sym) {
            if (cellule[0] != '\0')
              strncat(cellule, ",", sizeof(cellule) - strlen(cellule) - 1);
            strncat(cellule, _nom_etat(as, t.etat_arrive, tmp, sizeof(tmp)),
                    sizeof(cellule) - strlen(cellule) - 1);
          }
        }
        if (cellule[0] == '\0')
          strncpy(cellule, "--", sizeof(cellule) - 1);
  ...
  for (int e = 0; e < nb_etats; e++) {
    ...
    snprintf(tampon, sizeof(tampon), "%s %s", marqueur,
             _nom_etat(as, e, tmp, sizeof(tmp)));
